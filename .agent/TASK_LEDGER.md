...
| B5-0201 | OPEN | Headless smoke test: drive DeckLoader + GameState + one full AI turn with no GUI; fail loudly on exception | `b5ccg/src/b5ccg/engine/` (new test harness file only, no game-logic changes) | none | — |
| B5-0202 | OPEN | Audit `ai/` turn quality vs rulebook: report illegal or no-op AI moves, fix the smallest set | `b5ccg/src/b5ccg/ai/` | none | — |
